    ...
    <T title="How to Dispute on TransUnion — Free Guide" desc="Dispute inaccurate information with TransUnion online or by mail.">
      <div className="cr-card mb-8">
        ...
        <ol className="space-y-2 text-cr-muted text-sm">
          {['Go to transunion.com/dispute', 'Create account or sign in', 'Select the account to dispute', 'Choose dispute reason', 'Submit — TransUnion has 30 days under FCRA'].map((s, i) => <li key={i} className="flex items-start gap-2"><span className="text-cr-primary font-bold">{i + 1}. </span>{s}</li>)}
        </ol>
      </div>
      <div className="cr-card mb-8">
