...
export default function PayForDeletePage() {
  return (
    <T title="Pay for Delete Letter — Remove Collections by Paying" desc="How to negotiate collection removal with a pay-for-delete letter.">
      ...
      <div className="cr-card mb-8">
        ...
        <ol className="space-y-3">
          {['Send a debt validation letter first — if they can't verify the debt, you may not owe anything', 'Once validated, offer a settlement (typically 30-50% of the balance)', 'Explicitly request deletion as part of the agreement before paying', 'Get the agreement in writing before sending any payment', 'Pay with a method that leaves a paper trail', 'Send follow-up letters to bureaus if collector doesn\'t honor the agreement'].map((step, i) => (
            <li key={i} className="flex items-start gap-3"><span className="w-6 h-6 bg-cr-primary/20 rounded-full flex items-center justify-center flex-shrink-0 text-cr-primary font-bold text-sm">{i + 1}</span><span className="text-cr-muted text-sm">{step}</span></li>
          ))}
        </ol>
      </div>
      <div className="cr-card mb-8">
